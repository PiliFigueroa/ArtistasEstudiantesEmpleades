...
const cantidadDeArtistasPorInstrumento = (artistas) => {
  ...
  let artistasPorInstrumento = {}
  for (const instrumento of instrumentos) {
    artistasPorInstrumento[instrumento] = artistas.filter(artista => artista.instrumento === instrumento).length
  }
...
const artistaConMasEntradasVendidas = (artistas) => {
  ...
  for (const { nombre, ultimoRecital: { entradasVendidas }} of artistas) {
    if (entradas < entradasVendidas) {
      entradas = entradasVendidas
      artistaQueMasVendio = nombre
    }
  }
...
const artistaConMayorRecaudacion = (artistas) => {
  ...
  for (const { nombre, ultimoRecital: { entradasVendidas, costoEntradas } } of artistas) {
    const recaudadoPorArtista = entradasVendidas * costoEntradas
    if (mayorRecaudacion < recaudadoPorArtista) {
      mayorRecaudacion = recaudadoPorArtista
      artistaQueMasRecaudo = nombre
    }
  }
...
const artistasConDiscoEnAnio = (anio) => {
    return artistas.filter(({ discos }) => {
      return discos.some(disco => disco.anioLanzamiento === anio)
    }) 
}
...
const artistaConMasCopias = () => {
 let artistaConMasDiscos
 let cantDiscos = 0
 for (const { nombre, discos } of artistas) {
   if (cantDiscos < discos.length) {
     cantDiscos = discos.length
     artistaConMasDiscos = nombre
   }
 }
 return artistaConMasDiscos
}
